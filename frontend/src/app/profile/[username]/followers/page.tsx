  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        if (cancelled) return;
        setProfile(p);
        const list = await apiFetch<User[]>(`/api/users/${p.id}/followers`);
        if (cancelled) return;
        setFollowers(list);
      } catch {
  ...
  return (
    <main className="mx-auto max-w-3xl px-4 sm:px-6 lg:px-8 py-14">
      ...
      <SectionHeader
        ...
        title="Followers"
        description={`${profile.follower_count} ${
          profile.follower_count === 1 ? "person" : "people"
        } following`}
      />
      ...
      <div className="mt-10">
        {followers.length === 0 ? (
          <div className="rounded-2xl border border-dashed border-ink-3 bg-paper-1 p-12 text-center">
            <p className="font-display italic text-ink-7">No followers yet.</p>
          </div>
        ) : (
